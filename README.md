...
-VRSAVaultSignPKCS RSA签名数组算法,包括四个子函数，VbnRsaVault_GetPubParams、ApplyPKCS1SigningFormat、VbnRsaVault_ModExpPriv_clear、vMemrev. 其中的ApplyPKCS1SigningFormat只是逆序RSA数组然后连接一个固定数组,其他函数里面的VBN_MainLoop算法很变态,利用CFG调用上百个函数,只能用汇编抠出来用MASM32编译成一个dll供调用.最后生成256位数组.后来发现winipcsecproc.dll中都有函数原型,稍微变通一下就可以调用了.
