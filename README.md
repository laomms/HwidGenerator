...
-VRSAVaultSignPKCS RSA签名数组算法,包括四个子函数，VbnRsaVault_GetPubParams、ApplyPKCS1SigningFormat、VbnRsaVault_ModExpPriv_clear、vMemrev. VbnRsaVault_GetPubParams是申请一个256大小的数组空间，ApplyPKCS1SigningFormat是逆序RSA数组然后连接一个固定数组,VbnRsaVault_ModExpPriv_clear里的VBN_MainLoop算法很变态,利用CFG调用600多个函数,将这个256字节的数组进行加密算法。只能用汇编抠出来用MASM32编译成一个dll供调用.最后生成256位数组.发现winipcsecproc.dll中都有这些函数原型,但是测试调用后算法结果不一样.
